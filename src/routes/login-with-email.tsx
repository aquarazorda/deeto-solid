    ...
    <>
      ...
      <div class="flex h-screen flex-col items-center justify-center overflow-hidden bg-yellow-red bg-cover bg-no-repeat">
        <Form class="flex w-1/2 flex-col gap-6 rounded-no-left-top bg-yellow-bg p-12">
          ...
          <TextField.Root>
            <TextField.Input name='email' placeholder={t.EMAIL_ADDRESS()} class="w-full rounded-lg p-4 text-2xl font-medium shadow-primary-input"/>
          </TextField.Root>
          <Button.Root type='submit' class="ml-auto rounded-full bg-primary-purple px-6 py-4 font-semibold text-white">
            {t.CONTINUTE()}
          </Button.Root>
        </Form>
      </div>
