...
export default function VendorDashboard() {
  ...
  return (
    ...
    <Title>{t.APP_NAME()} - {t.REFERENCES()}</Title>
    <h1 class="font-inter text-5xl font-bold text-primary-purple">{t.REFERENCES()}</h1>
              ...
              {(item) => (
                <div class="flex min-w-[16rem] flex-1 flex-col rounded-2xl rounded-tl-none bg-white">
                  <div class="flex flex-col gap-4 p-4 shadow-ref-box">
                    <div>
                      <div class="flex gap-4">
                        <Avatar firstName={item.AuthenticatedUsers.firstName} lastName={item.AuthenticatedUsers.lastName} />
                        <div class="flex flex-col justify-center font-inter font-semibold text-purple-dark">
                          <span>{item.AuthenticatedUsers.firstName} {item.AuthenticatedUsers.lastName}</span>
                  ...
                  <div class="rounded-b-2xl bg-secondary-green py-[0.125rem] text-center text-white shadow-ref-box">
                    <div class="font-inter text-sm font-medium uppercase">{item.AuthenticatedUsers.userStatus}</div>
                  </div>
                </div>
              )}
            </For>
